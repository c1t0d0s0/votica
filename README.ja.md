...
- **日程調整モード (調整さん風出欠表)**: 飲み会やミーティング等の候補日程を入力し、参加者が ◯・△・✗・？ で出欠を回答できる日程調整を作成可能。ログイン不要で名前入力のみで回答でき、参加者全員でリアルタイム出欠マトリクス表を閲覧可能
...
- **Google認証による厳格な1人1票保証 & ログイン不要設定**: Google認証による1人1票保証と、参加ハードルを下げるログイン不要（ニックネーム申告）モードの両方に対応
- **柔軟な選択肢 & 上限設定**: 選択肢は最大20個（日程調整は最大30日程）まで設定可能。テキストエリアでの一括入力やカレンダー・クイック追加ツールにも対応
- **誰でも投票・日程調整を作成可能**: Googleログインユーザーなら誰でも新しい投票・日程調整を作成し、管理者として管理可能
- **結果の公開/非公開制御**: 全体公開・管理者限定公開の切り替え可能
- **出欠サマリーのテキストコピー & CSVエクスポート**: 日程調整結果をLINEやSlackにワンクリックでコピーしたり、Excel用CSVとしてダウンロード可能
- **リアルタイム集計**: Firestore のリアルタイム同期により、出欠状況や投票グラフが即時反映
